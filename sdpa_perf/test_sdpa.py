...
def main(args):
    ...
    print(f"batch_size:{batch_size}, seqlen:{max_sequence_len}, emb_dim:{embed_dimension}")
    ...
    try:
        ...
        default_time = benchmark_torch_function_in_microseconds(F.scaled_dot_product_attention, query, key, value)
        ...
        with sdp_kernel(**backend_map[SDPBackend.EFFICIENT_ATTENTION]):
            try:
                memory_efficient_time = benchmark_torch_function_in_microseconds(F.scaled_dot_product_attention, query, key, value)
                max_time = max(max_time, memory_efficient_time)
                min_time = min(min_time, memory_efficient_time)
            except RuntimeError:
                print("EfficientAttention is not supported. See warnings for reasons.")
    except torch.cuda.OutOfMemoryError:
        print(f"batch_size:{batch_size}, seq_len:{max_sequence_len}, num_heads:{num_heads} OutofMemory")
        return
    ...
    try:
        ...
        flash_att2_time = benchmark_torch_function_in_microseconds(flash_attn_func, query, key, value, dropout_p=0.0, softmax_scale=None, causal=False)
        ...
        def flash_varlen(q, k, v, device):
            ...
            cu_seqlens_k = cu_seqlens_q = torch.arange(0, (batch_size + 1) * seqlen_q, step=seqlen_q, dtype=torch.int32, device=device)
            output = flash_attn_varlen_func(q, k, v, cu_seqlens_q, cu_seqlens_k, seqlen_q, seqlen_q, causal=True)
            return output

        flash_att_varlen_time = benchmark_torch_function_in_microseconds(flash_varlen, query, key, value, device)
    ...
    try:
        ...
        xformers_time = benchmark_torch_function_in_microseconds(xops.memory_efficient_attention, query, key, value)
    ...
    #main(args)
    gpu_type = os.popen("nvidia-smi -i 0 -q | grep 'Product Name' | awk -F ' : ' '{ print $2 }'").readlines()[0].strip()
    ...
    print(f"flashattention varlen implement runtime {flash_att_varlen_time} ms")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_heads", type=int, default=32, help="the number of heads")
