...
def test_infer_performance(model, model_name, batch_size, num_data, input_shape=None, use_fp16=False, use_mixed_precision=True, mode="native"):
    model.eval()
    model.cuda(0)
    print(f'warm up')
    ...
    logging.info(f"warm up with {mode} {TORCH_NVFUSER_LABEL}")
    data = torch.rand(B, C, H, W, dtype=dtype, device=0)
    if mode == "script":
        model = torch.jit.script(model)
        logging.info(f"success script model")
    elif mode == "trace":
        model = torch.jit.trace(model, example_inputs=(data))
        logging.info(f"success trace model")
    else:
        pass
    ...
    print('[{}_{}x{}] time: {:.4f} ms / image'.format(model_name, W, H, elapsed_time / num_data * 1000))
    logging.info('[{}_{}x{}][{}] time: {:.4f} ms / image'.format(model_name, W, H, mode, elapsed_time / num_data * 1000))


def test_bert_infer_performance(model, tokenizer, model_name, batch_size, num_data, max_length=128, use_mixed_precision=True, mode="native"):
    model.eval()
    model.cuda(0)

    inputs = tokenizer(["Welcome to deepindeed!"] * batch_size,
    ...
    if mode == "script":
        ...
        model = torch.jit.trace(model, example_inputs=(inputs["input_ids"].cuda(0), inputs["attention_mask"].cuda(0), inputs["token_type_ids"].cuda(0)), strict=False)
        logging.info(f"success trace model")
    else:
        pass
    ...
    logging.info('[{}_{}][{}] time: {:.4f} ms / image'.format(model_name,
          max_length, mode, elapsed_time / num_data * 1000))